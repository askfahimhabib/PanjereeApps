...
import { mockClasses, mockGroups, mockSections } from './mockData'
...
export function useClassDetail(classId: string | undefined) {
  const [baseClasses, setClasses] = useState<ClassItem[]>(mockClasses)
  const [baseSections, setSections] = useState<Section[]>(mockSections)
  const [refreshKey, setRefreshKey] = useState(0) // Used to force re-computation when MOCK_STUDENTS mutates
  
  // Dynamically compute group stats based on MOCK_STUDENTS and baseSections
  const groups: ClassGroup[] = useMemo(() => {
    return mockGroups.filter(g => g.classId === classId).map(g => {
      const groupStudents = MOCK_STUDENTS.filter(s => s.classId === classId && s.groupId === g.name)
  ...
  const sections = useMemo(() => {
    return baseSections.map(sec => {
      const secStudents = MOCK_STUDENTS.filter(s => {
  ...
  const getGroupStudents = useCallback(
    (groupId: string) => {
      ...
      return classStudents.filter(s => {
        const student = MOCK_STUDENTS.find(ms => ms.id === s.id)
  ...
  const addSection = useCallback(
    (formData: AddSectionFormData, groupId?: string, groupName?: string) => {
      ...
      const existingSections = mockSections.filter(s => s.classId === classId && s.groupId === groupId)
        ...
        MOCK_STUDENTS.forEach(student => {
          if (student.classId === classId) {
            if (!groupId || student.groupId === groupName) {
              ...
              student.sectionName = newSection.name
            }
          }
        })
      }
  ...
  const assignStudentToSection = useCallback((studentId: string, toSectionId: string) => {
    const section = mockSections.find(s => s.id === toSectionId)
    if (!section) return

    const studentIndex = MOCK_STUDENTS.findIndex(s => s.id === studentId)
    if (studentIndex > -1) {
      MOCK_STUDENTS[studentIndex].sectionId = section.id
      MOCK_STUDENTS[studentIndex].sectionName = section.name
