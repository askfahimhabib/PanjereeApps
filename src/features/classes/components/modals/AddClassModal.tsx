  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-lg mx-4 bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-5 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Select a class and configure settings</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                ...
                <button
                  ...
                  className={`relative py-2.5 px-2 rounded-xl border text-sm font-medium transition-all ${
                    selectedPreset === preset.numericName
                      ...
                      : 'bg-slate-800/60 border-slate-700/50 text-slate-300 hover:border-blue-500/50 hover:text-white'
                  }`}
                >
                  {preset.label}
                  {preset.hasGroups && (
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-medium transition-all ${
                    form.shift === s.value
                      ...
                      : 'bg-slate-800/60 border-slate-700/50 text-slate-300 hover:border-indigo-500/50 hover:text-white'
                  }`}
                >
                  {s.icon}
                  {s.label}
          ...
          >
            <div className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
              form.hasGroups ? 'bg-emerald-600 border-emerald-500' : 'border-slate-600'
            }`}>
              {form.hasGroups && (
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-200">Has Academic Groups</span>
              </div>
              ...
              <div className="mt-3 space-y-3">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1.5">
                    Monthly Tuition Fee (৳) <span className="text-slate-500 font-normal">(Optional)</span>
                  </label>
                  <input
                    type="number"
                    min={0}
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-slate-800 bg-slate-900/80">
