        ...
        <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4 text-center">
              <p className="text-xs text-slate-500 mb-1">Current Session</p>
              <p className="text-lg font-bold text-slate-200">{FROM_YEAR}</p>
            </div>
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4 text-center">
              <p className="text-xs text-slate-500 mb-1">New Session</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center gap-3 p-3 bg-blue-500/5 border border-blue-500/20 rounded-xl">
              ...
              <div>
                <p className="text-xs text-slate-500">Total Promoting</p>
          ...
          {classes.length > 0 && (
            <div>
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Promotion Map</p>
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                {classes.map(cls => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-slate-300 font-medium w-28 shrink-0 truncate">{cls.name}</span>
                      <ChevronRight size={13} className="text-slate-600 shrink-0" />
                      <span className={`flex-1 ${
                        type === 'graduate' ? 'text-purple-400' :
                        type === 'hsc'     ? 'text-amber-400' : 'text-emerald-400'
                      }`}>{to}</span>
                      <span className="text-xs text-slate-500 shrink-0">{active} students</span>
                    </div>
                  )
                })}
              </div>
            ...
            {([
              ...
              >
                <div className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                  checklist[item.key] ? 'bg-emerald-600 border-emerald-500' : 'border-slate-600'
                }`}>
                  {checklist[item.key] && (
                ...
                <div>
                  <p className={`text-sm font-medium ${checklist[item.key] ? 'text-emerald-300' : 'text-slate-300'}`}>
                    {item.label}
                  </p>
    ...
    return (
      <Dialog onClose={() => {}}>
        <div className="p-12 flex flex-col items-center text-center gap-4">
          ...
          <div>
            <p className="text-lg font-semibold text-slate-200 mb-1">Running Rollover...</p>
            <p className="text-sm text-slate-400">Promoting {totalActive - graduating} students to new classes</p>
          </div>
  ...
  return (
    <Dialog onClose={handleClose}>
      <div className="p-12 flex flex-col items-center text-center gap-4">
        ...
        <div>
          <p className="text-lg font-semibold text-slate-200 mb-1">Rollover Complete!</p>
          <p className="text-sm text-slate-400">
          ...
          <p className="text-xs text-slate-500 mt-2">New session: <span className="text-slate-300">{TO_YEAR}</span></p>
        </div>
        <button
          onClick={handleClose}
...
function Dialog({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-lg mx-4 bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden">
        {children}
      </div>
    </div>
  )
