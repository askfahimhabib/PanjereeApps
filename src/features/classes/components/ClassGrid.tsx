...
export function ClassGrid({ classes, onToggleActive }: ClassGridProps) {
  if (classes.length === 0) {
    return (
      <div className="text-center py-16 text-slate-500 border border-dashed border-slate-700/50 rounded-xl">
        No classes found.
      </div>
    )
  }
        ...
        return (
          <div
            ...
            className={`group relative overflow-hidden rounded-xl border transition-all duration-300 ${
              inactive
                ? 'border-slate-800/60 bg-slate-900/40 opacity-50 grayscale-[30%]'
                : 'border-slate-700/50 bg-slate-800/50 hover:bg-slate-800/80 hover:border-slate-600 hover:shadow-lg hover:shadow-blue-900/20'
            }`}
          >
            {/* Inactive overlay label */}
            {inactive && (
              <div className="absolute top-2.5 left-2.5 z-10 flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-800 border border-slate-700 text-[10px] font-semibold text-slate-500">
                <PowerOff size={9} />
                Disabled
              </div>
            )}
            ...
            <div className="p-5">
              ...
              <div className="flex justify-between items-start mb-3">
                <div className={inactive ? 'mt-5' : ''}>
                  ...
                  <span className="text-xs text-slate-400">{cls.academicYear} · {cls.shift}</span>
                </div>
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg border transition-all ${
                        inactive
                          ...
                          : 'border-slate-700/50 bg-slate-900/50 text-slate-500 hover:text-red-400 hover:border-red-500/30 hover:bg-red-500/10'
                      }`}
                    >
              ...
              {cls.hasGroups && cls.groups && cls.groups.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-4 pt-3 border-t border-slate-700/50">
                  {cls.groups.map(g => {
              ...
              <Link
                ...
                className={`flex items-center justify-between w-full px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                  inactive
                    ...
                    : 'bg-slate-700/50 hover:bg-blue-600 text-slate-300 hover:text-white group-hover:bg-blue-600 group-hover:text-white'
                }`}
