      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-900/60 border-b border-slate-700/50">
            <tr>
              <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Teacher</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Designation</th>
          ...
          <tbody className="divide-y divide-slate-700/30">
            {teachers.map(teacher => (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <button
                        ...
                        className="font-medium text-slate-200 hover:text-indigo-400 transition-colors leading-tight flex items-center gap-1 group"
                      ...
                      {teacher.nameBangla && (
                        <p className="text-xs text-slate-500 mt-0.5 truncate">{teacher.nameBangla}</p>
                      )}
                ...
                {/* Designation & Type */}
                <td className="px-4 py-3">
                  <p className="text-slate-300 text-sm">{DESIGNATION_LABELS[teacher.designation]}</p>
                  <p className="text-slate-500 text-xs mt-0.5">{EMPLOYMENT_TYPE_LABELS[teacher.employmentType]}</p>
                </td>
                ...
                <td className="px-4 py-3">
                  {teacher.department ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-slate-400 text-xs">{DEPARTMENT_LABELS[teacher.department]}</span>
                    </div>
                  ) : (
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-slate-400 text-xs font-mono">{teacher.phone}</span>
                  </div>
                  {teacher.email && (
                    <p className="text-xs text-slate-600 mt-0.5 truncate max-w-[160px]">{teacher.email}</p>
                  )}
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-700/50">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 text-xs text-slate-400 hover:text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-slate-700 rounded-lg transition-colors border border-slate-700"
          >
