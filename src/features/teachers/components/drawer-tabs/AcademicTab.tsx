    ...
    <div className="space-y-6">
          ...
          : (
            <div className="space-y-3">
              {teacher.qualifications.map(q => (
                <div key={q.id} className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-semibold text-slate-200">{q.degree} — {q.subject}</p>
                      <p className="text-sm text-slate-400 mt-0.5">{q.institution}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{q.university}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-xs text-emerald-400 font-medium">{q.result}</p>
                      <p className="text-xs text-slate-500 mt-1">{q.passingYear}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {teacher.specialization && (
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-300">{teacher.specialization}</p>
        </div>
      )}
          ...
          : (
            <div className="space-y-3">
              {teacher.previousExperience.map(e => (
                <div key={e.id} className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-3 flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-medium text-slate-200">{e.organization}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{e.designation}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs text-slate-300">{e.fromYear} — {e.toYear || 'Present'}</p>
