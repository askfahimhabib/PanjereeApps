...
const sectionTitleCls = 'text-sm font-semibold text-slate-300 flex items-center gap-2'
    ...
    <div className="space-y-5">
        ...
        <div className="space-y-3">
          {data.qualifications.map((q, idx) => (
            <div key={idx} className="bg-slate-800/60 border border-slate-700 rounded-lg p-3 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500 font-medium">Qualification #{idx + 1}</span>
      ...
      <div className={sectionCls}>
        ...
        {data.certifications.length === 0
          ? <p className="text-xs text-slate-500 italic">No certifications added. Click Add to add one.</p>
          : (
            <div className="space-y-3">
              {data.certifications.map((c, idx) => (
                <div key={idx} className="bg-slate-800/60 border border-slate-700 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-slate-500 font-medium">Certificate #{idx + 1}</span>
      ...
      <div className={sectionCls}>
        ...
        {data.previousExperience.length === 0
          ? <p className="text-xs text-slate-500 italic">No previous experience added.</p>
          : (
            <div className="space-y-3">
              {data.previousExperience.map((e, idx) => (
                <div key={idx} className="bg-slate-800/60 border border-slate-700 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-slate-500 font-medium">Experience #{idx + 1}</span>
