  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h4 className="text-xs font-semibold uppercase tracking-widest text-slate-500">Daily Attendance (Last 30 Days)</h4>
      ...
      {/* Bar chart */}
      <div className="flex items-end gap-1 h-32 bg-slate-800/30 rounded-xl p-3 border border-slate-700/40">
        {daily.map((d, i) => {
          ...
          return (
            ...
            >
              {/* Tooltip */}
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-700 border border-slate-600 text-[10px] text-slate-200 rounded-lg px-2 py-1.5 whitespace-nowrap z-10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl">
  ...
  return (
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-700/50 bg-slate-800/60">
        <h4 className="text-xs font-semibold uppercase tracking-widest text-slate-400">Attendance by Class</h4>
      </div>
      <div className="divide-y divide-slate-800/60">
        {sorted.map((cls, i) => (
          <div key={i} className="flex items-center gap-4 px-4 py-3">
            <span className="text-sm text-slate-300 min-w-[120px]">{cls.className}</span>
