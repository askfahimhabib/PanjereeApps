...
function MonthlyBarChart({ monthly }: { monthly: PaymentSummary['monthly'] }) {
  ...
  return (
    <div className="bg-slate-800/30 border border-slate-700/40 rounded-2xl p-5 space-y-4">
      <h4 className="text-xs font-semibold uppercase tracking-widest text-slate-500">Monthly Collection (2024)</h4>
        ...
        {monthly.map((m, i) => {
          ...
          return (
            <div key={i} className="group flex-1 flex flex-col items-center gap-1.5 relative">
              {/* Tooltip */}
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-700 border border-slate-600 text-[10px] text-slate-200 rounded-lg px-2.5 py-2 whitespace-nowrap z-10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl">
                <p className="font-semibold text-emerald-400">{formatCurrency(m.total)}</p>
                <p className="text-slate-400">{m.count} transactions</p>
              </div>
              {/* Bar */}
              <div
              ...
              />
              {/* Label */}
              <span className="text-[9px] text-slate-500">{m.monthName}</span>
            </div>
          )
        })}
      </div>
  ...
  return (
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-700/50 bg-slate-800/60">
        <h4 className="text-xs font-semibold uppercase tracking-widest text-slate-400">Revenue by Fee Type</h4>
      </div>
          ...
          return (
            <div key={i} className="flex items-center justify-between px-4 py-3">
              ...
              <div className="text-right">
                <p className="text-sm font-semibold text-slate-200">{formatCurrency(f.total)}</p>
