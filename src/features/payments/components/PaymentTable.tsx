...
import { mockClasses } from '@/features/classes/mockData'
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-8 pr-3 py-2 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-emerald-500"
          />
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-800/60 border-b border-slate-700">
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">Invoice</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Student</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider hidden md:table-cell">Items</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider hidden lg:table-cell">Method</th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Amount</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider hidden sm:table-cell">Date</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Status</th>
                    ...
                    <tr key={i} className="animate-pulse">
                      {Array.from({ length: 8 }).map((_, j) => (
                        <td key={j} className="px-4 py-3">
                          <div className="h-4 rounded bg-slate-800/60 w-20" />
                        </td>
                      ))}
                    </tr>
                  ))
                    ...
                    return (
                      <tr key={record.id} className="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <p className="text-slate-200 font-medium text-xs">{record.student_name}</p>
                          <p className="text-slate-500 text-[10px]">Roll {record.roll_number} · {record.class_name ?? '—'}</p>
                        </td>
                        <td className="px-4 py-3 hidden md:table-cell">
                          <p className="text-slate-400 text-xs max-w-[160px] truncate" title={itemSummary}>
                            {itemSummary}
                          </p>
                        </td>
                        ...
                        <td className="px-4 py-3 text-slate-400 text-xs whitespace-nowrap hidden sm:table-cell">
                          {paidDate}
                        </td>
                        <td className="px-4 py-3">
