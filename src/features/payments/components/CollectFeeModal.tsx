...
export function CollectFeeModal({ open, preselectedStudent, onClose }: Props) {
  ...
  if (successStep && lastRecord) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm shadow-2xl p-8 text-center">
          <div className="w-16 h-16 rounded-full bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center mx-auto mb-4">
            <CheckCircle2 size={32} className="text-emerald-400" />
          </div>
          <h3 className="text-xl font-bold text-slate-100 mb-1">Payment Collected!</h3>
          <p className="text-slate-400 text-sm mb-1">{lastRecord.student_name}</p>
          <p className="text-xs text-slate-500 mb-4">{lastRecord.invoice_number}</p>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onClose}
              className="flex-1 py-2.5 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 text-sm font-medium transition-all"
            >
              Done
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[92vh]">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 flex-shrink-0">
          <div>
            <h3 className="text-lg font-bold text-slate-100">Collect Fee</h3>
            {selectedStudent && (
        ...
        {/* Step Indicator */}
        <div className="flex items-center px-6 py-3 border-b border-slate-800/50 gap-2 flex-shrink-0">
          {STEPS.map((label, i) => {
            const stepNum = i + 1
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                ...
                <span className={`text-xs ${isActive ? 'text-slate-200' : isDone ? 'text-emerald-400' : 'text-slate-600'}`}>{label}</span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-9 pr-3 py-2.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  {studentResults.map(s => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 bg-slate-800/60 hover:bg-slate-800 border border-slate-700/50 hover:border-emerald-500/30 rounded-xl text-left transition-all"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-200">{s.fullNameEn}</p>
                        <p className="text-xs text-slate-500">Roll {s.rollNumber} · {s.className ?? s.classId}</p>
                      </div>
              ...
              {search.length >= 2 && studentResults.length === 0 && (
                <p className="text-center text-slate-600 text-sm py-6">No active students found</p>
              )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-xs text-slate-400">Fee items for this invoice</p>
                <button
              ...
              <div className="space-y-3">
                {lineItems.map((item, idx) => (
                  <div key={item.id} className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3 space-y-2">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[10px] text-slate-500 mb-1">Fee Type</label>
                        <select
                          value={item.fee_type}
                          ...
                          className="w-full bg-slate-900 border border-slate-700 rounded-md px-2 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-emerald-500"
                        >
              ...
              <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">Discount Amount (৳)</label>
                    <input
                      type="number"
                      min={0}
                      max={subtotal}
                      ...
                      placeholder="0"
                      className="w-full bg-slate-900 border border-slate-700 rounded-md px-2 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div>
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="flex justify-between text-base font-bold text-emerald-400 pt-1 border-t border-slate-700">
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              {paymentMethod !== 'CASH' && (
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1.5">Transaction ID</label>
                  <input
                    type="text"
                    value={transactionId}
                    onChange={e => setTransactionId(e.target.value)}
                    placeholder="e.g. BKD2026XXXX"
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-emerald-500"
                  />
                </div>
              )}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-emerald-500 resize-none"
                />
              </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white border border-slate-700 hover:border-slate-600 transition-all"
          >
            <ChevronLeft size={15} />
            {step === 1 ? 'Cancel' : 'Back'}
          </button>
