    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 flex-shrink-0">
          <div>
            <h3 className="text-base font-bold text-slate-100">Invoice Preview</h3>
            <p className="text-xs text-slate-500 mt-0.5">{record.invoice_number}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="bg-white text-slate-900 rounded-xl overflow-hidden shadow-lg text-sm">
            ...
            <div className="bg-slate-900 text-white px-6 py-5 text-center">
              ...
              <p className="text-[10px] text-slate-400 mt-1">Official Fee Receipt</p>
            </div>

            <div className="bg-slate-800 text-center py-2">
            ...
            <div className="px-5 py-4 border-b border-dashed border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-1 mt-2 text-xs text-slate-500">
                <span>Roll: <strong className="text-slate-700">{record.roll_number}</strong></span>
                <span>Class: <strong className="text-slate-700">{record.class_name ?? '—'}</strong></span>
                <span>Date: <strong className="text-slate-700">{paidDate}</strong></span>
                <span>
                  Status:{' '}
            ...
            {/* Fee Items */}
            <div className="px-5 py-4 border-b border-dashed border-slate-200">
              <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-3 font-semibold">Fee Details</p>
                  ...
                  return (
                    <div key={i} className="flex justify-between items-start">
                      <div>
                        ...
                        {period && <p className="text-[10px] text-slate-400">{period}</p>}
                      </div>
                      <p className="text-xs font-bold text-slate-800 whitespace-nowrap">{formatCurrency(item.amount)}</p>
                    </div>
                  )
                })}

                {record.discount_amount > 0 && (
                  <div className="flex justify-between items-start pt-2 border-t border-dashed border-slate-200">
                    <div>
                      <p className="text-xs font-medium text-red-600">Discount / Waiver</p>
                      {record.waiver_reason && <p className="text-[10px] text-slate-400">{record.waiver_reason}</p>}
                    </div>
            ...
            <div className="px-5 py-4 grid grid-cols-2 gap-4">
              <div className="text-center pt-6 border-t border-slate-300">
                <p className="text-[10px] text-slate-400">Student / Guardian</p>
              </div>
              <div className="text-center pt-6 border-t border-slate-300">
                <p className="text-[10px] text-slate-400">Collector: {record.collected_by}</p>
              </div>
            </div>

            <div className="text-center pb-3">
              <p className="text-[9px] text-slate-300 tracking-widest">PANJEREE COACHING CENTER · OFFICIAL RECEIPT</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-slate-800 flex-shrink-0">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-400 hover:text-white hover:border-slate-600 text-sm transition-all"
          >
            Close
          </button>
          <button
