...
export function ResultsEntryModal({ open, exam, existingResults, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-2 sm:p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[95vh]">
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-slate-800 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Enter marks below · Default Total: <strong className="text-slate-300">{exam.total_marks}</strong>
              {exam.pass_marks && <span> · Default Pass: <strong className="text-slate-300">{exam.pass_marks}</strong></span>}
              <span className="ml-2 text-slate-600">(per-subject marks override if set)</span>
            </p>
          </div>
          <div className="flex items-center gap-3 ml-4">
            {/* Progress */}
            <span className="text-xs text-slate-500">{filledCells}/{totalCells} filled</span>
            ...
            return (
              <div key={subjectId}>
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-[10px] text-slate-600">/ {effectiveTotalMarks(subjectId)}</span>
                </div>

                {/* Column headers */}
                <div className="grid grid-cols-[2rem_1fr_5rem_5rem_4rem_4rem] gap-2 text-[10px] font-semibold text-slate-500 uppercase tracking-wider px-3 mb-1">
                  <span>#</span>
                  <span>Student</span>
                  <span>Marks</span>
                ...
                <div className="space-y-1.5">
                  {rows.map(row => {
                    ...
                    const gradeColor = gradeResult
                      ? GRADE_COLORS[gradeResult.grade] ?? 'text-slate-400'
                      : ''
                    ...
                    return (
                      ...
                      >
                        {/* Roll */}
                        <span className="text-xs text-slate-500 font-mono">{row.rollNumber}</span>

                        {/* Name */}
                        <span className="text-sm text-slate-200 truncate">{row.studentName}</span>
                        ...
                        <input
                          ...
                          placeholder="—"
                          className="w-full bg-slate-900 border border-slate-700 rounded-md px-2 py-1 text-sm text-slate-200 text-center focus:outline-none focus:border-purple-500 disabled:opacity-30 disabled:cursor-not-allowed"
                        />

                        {/* Total marks (static) */}
                        <span className="text-xs text-slate-600 text-center">/ {effectiveTotalMarks(subjectId)}</span>

                        {/* Grade badge */}
                        <span className={`text-[11px] font-bold text-center px-1.5 py-0.5 rounded border ${gradeColor || 'text-slate-600 border-transparent'}`}>
                          ...
                          <button
                            ...
                            className={`w-6 h-6 rounded-md border text-xs font-bold transition-all ${
                              row.isAbsent
                                ...
                                : 'border-slate-600 text-slate-600 hover:border-red-500/50'
                            }`}
                          >
                            A
                          </button>
