...
function MiniGradeBar({ counts, total }: { counts: Record<string, number>; total: number }) {
  ...
  if (total === 0) return <span className="text-xs text-slate-600">No results</span>
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-slate-700/50 overflow-hidden">
          {/* Table Header */}
          <div className="grid grid-cols-[1fr_100px_100px_120px_140px] gap-3 items-center px-5 py-3 bg-slate-800/60 border-b border-slate-700/50">
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-500">Exam Name</span>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 text-center">Scope</span>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 text-center">Subjects</span>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 text-center">Grade Dist.</span>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 text-right">Actions</span>
          </div>
          ...
          <div className="divide-y divide-slate-800/60">
            {filtered.map(exam => {
              ...
              return (
                <div
                  key={exam.id}
                  className="grid grid-cols-[1fr_100px_100px_120px_140px] gap-3 items-center px-5 py-4 hover:bg-slate-800/30 transition-colors"
                >
                  {/* Name */}
                  ...
                  {/* Subjects count */}
                  <div className="text-center">
                    <span className="text-sm font-bold text-slate-300">{scheduleCount}</span>
                    <span className="text-xs text-slate-600 ml-0.5">subj.</span>
                  </div>
                  ...
                  <div className="px-2">
                    {scheduleCount > 0 ? (
                      ...
                      <span className="text-xs text-slate-700 italic">No schedule</span>
                    )}
                  </div>
        ...
        {exams.length > completedExams.length && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-800/30 border border-slate-700/40">
            ...
            <p className="text-xs text-slate-500">
              ...
              Mark exams as <strong className="text-slate-400">Completed</strong> from the Exams tab to see results here.
            </p>
          </div>
        )}
      </div>
