  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="bg-slate-900/50 border border-slate-700/50 p-3 rounded-xl flex flex-col items-center">
          <span className="text-2xl font-bold text-slate-200">{total}</span>
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold mt-0.5">Total</span>
        </div>
          ...
          {recentRecords.length === 0 ? (
            ...
            <div className="divide-y divide-slate-700/30 max-h-72 overflow-y-auto">
              {recentRecords.map(r => {
                ...
                return (
                  <div key={r.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-slate-800/30 transition-colors">
              ...
              <div key={i} className="px-4 py-3 hover:bg-slate-800/20 transition-colors">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-xs font-bold ${
                    m.rate === null ? 'text-slate-600' :
                    m.rate >= 75 ? 'text-emerald-400' : m.rate >= 50 ? 'text-amber-400' : 'text-red-400'
                  }`}>{m.rate !== null ? `${m.rate}%` : '—'}</span>
                </div>
                {m.total > 0 ? (
                  <>
                    <div className="w-full bg-slate-700/40 rounded-full h-1.5 mb-1.5">
                  ...
                  <p className="text-[10px] text-slate-600">No records this month</p>
                )}
              </div>
            ))}
          </div>
