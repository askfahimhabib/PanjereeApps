    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 shrink-0">
          <div>
            <h2 className="text-lg font-semibold text-slate-100">{isEdit ? 'Edit Student' : 'Add New Student'}</h2>
            <p className="text-xs text-slate-400 mt-0.5">Step {currentStep} of {STEPS.length}: {STEPS[currentStep - 1].label}</p>
          </div>
            ...
            {STEPS.map((step, idx) => (
              <div key={step.num} className="flex items-center flex-1">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className={`text-xs mt-1 whitespace-nowrap ${currentStep === step.num ? 'text-blue-400' : currentStep > step.num ? 'text-emerald-400' : 'text-slate-500'}`}>
                    {step.short}
                  </span>
                </div>
                {/* Connector line */}
