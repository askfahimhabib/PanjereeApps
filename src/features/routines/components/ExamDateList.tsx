  ...
  return (
    <div className="space-y-6">
      {Object.entries(byMonth).map(([month, items]) => (
        <div key={month}>
          {/* Month label */}
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3 pl-1">
            {month !== 'N/A'
...
function ExamRow({ routine, onClick }: { routine: Routine; onClick?: () => void }) {
  ...
  return (
    <div
      ...
      `}>
      {/* Date Column */}
      <div className={`flex-shrink-0 w-20 text-center py-2 px-1 rounded-lg ${isInactive ? 'bg-slate-800/40' : 'bg-slate-900/60'}`}>
        {routine.specific_date ? (
          <>
            <p className="text-xl font-bold text-slate-200 leading-none">
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-semibold text-sm mb-1 ${isInactive ? 'line-through text-slate-500' : 'text-slate-200'}`}>
