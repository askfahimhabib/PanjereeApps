...
export function AttendanceRow({
  ...
  return (
    ...
    `}>
      {/* Roll */}
      <span className="w-8 text-center text-sm font-bold text-slate-500 flex-shrink-0">
        {rollNumber}
      </span>

      {/* Name */}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-slate-200 truncate">{fullNameEn}</p>
        <p className="text-xs text-slate-500 truncate">{fullNameBn}</p>
      </div>
          ...
          <button
            ...
            className={`
              ...
              ${status === s
                ...
                : 'border-slate-700 text-slate-500 hover:border-slate-500 hover:text-slate-300 bg-slate-900'
              }
            `}
          >
