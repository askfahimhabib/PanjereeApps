...
export function TeacherProfile() {
  ...
  const teacher = useMemo(() => teacherStore.getOne(teacherId ?? ''), [teacherId])

  if (!teacher) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-slate-600">
        <User size={48} className="mb-4 opacity-20" />
        <p className="text-lg font-medium text-slate-500">Teacher not found</p>
      ...
      <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl overflow-hidden">
        ...
        <div className="px-6 pb-5">
          ...
          <div className="space-y-2">
            <div>
              <h1 className="text-2xl font-bold text-slate-100">{teacher.fullName}</h1>
              {teacher.nameBangla && <p className="text-sm text-slate-400 mt-0.5">{teacher.nameBangla}</p>}
              <p className="text-sm text-slate-500 mt-1">{DESIGNATION_LABELS[teacher.designation]}</p>
            </div>
