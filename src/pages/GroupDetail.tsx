...
export function GroupDetail() {
  ...
  if (!classData || !groupData) {
    return <div className="p-6 text-center text-slate-400">Group not found</div>
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-800/50 border border-slate-700/50 rounded-xl p-5">
        <div>
          ...
          <p className="text-sm text-slate-400">
            {classData.name} &nbsp;·&nbsp;
            <span className="text-slate-300">{groupData.totalStudents}</span> Students &nbsp;·&nbsp;
            <span className="text-slate-300">{groupSections.length}</span> Sections
          </p>
        </div>
