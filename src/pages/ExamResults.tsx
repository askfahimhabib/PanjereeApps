      ...
      {grades.map(g => {
        ...
        return (
          <div key={g} className="flex items-center gap-3 text-xs">
            <span className="w-6 text-right text-slate-400 font-mono font-bold">{g}</span>
            ...
            <span className="w-10 text-right text-slate-500 font-mono">{count}</span>
          </div>
        )
      })}
    </div>
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="space-y-2 animate-pulse">
          {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-12 rounded-xl bg-slate-800/50" />)}
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_220px] gap-4">
          ...
          <div className="rounded-xl border border-slate-700/50 overflow-hidden">
            {/* Table header */}
            <div className="grid grid-cols-[3rem_1fr_6rem_5rem_5rem_4rem_2.5rem] gap-2 items-center px-4 py-3 bg-slate-800/70 border-b border-slate-700/50">
              {['Roll', 'Student', 'Marks', `/ ${effectiveTotalMarks}`, 'Grade', 'GPA', ''].map((h, i) => (
                <span key={i} className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 text-center first:text-left">{h}</span>
              ))}
            </div>
            <div className="divide-y divide-slate-800/60">
              {sortedRows.map(row => {
                ...
                return (
                  <div
                    key={row.id}
                    className={`grid grid-cols-[3rem_1fr_6rem_5rem_5rem_4rem_2.5rem] gap-2 items-center px-4 py-3 hover:bg-slate-800/40 transition-colors ${
                      row.is_absent ? 'opacity-50' : isFail ? 'bg-red-950/20' : ''
                    }`}
                  >
                    <span className="text-xs font-mono text-slate-400">{row.roll_number}</span>
                    <span className="text-sm text-slate-200 truncate font-medium">{row.student_name}</span>
                    <span className="text-sm text-center font-mono font-bold text-slate-200">
          ...
          <div className="space-y-4">
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 mb-4">Grade Distribution</p>
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4 space-y-2">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 mb-3">Actions</p>
              <button
              ...
              {allResults.length > 0 && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium border border-slate-700 text-slate-400 hover:text-white hover:border-slate-500 transition-all"
                >
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-slate-200 truncate">{exam.name}</p>
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span className="text-[10px] font-medium text-slate-400 bg-slate-700/60 px-2 py-0.5 rounded-md">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Exam Results</h1>
          <p className="text-sm text-slate-500 mt-1">View, publish and print results for completed exams</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="space-y-2 animate-pulse">
          {Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-20 rounded-xl bg-slate-800/50" />)}
        </div>
      ) : completedExams.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-slate-600">
          <BarChart3 size={56} className="mb-4 opacity-20" />
          <p className="text-base font-medium text-slate-500">No completed exams yet</p>
          <p className="text-sm text-slate-600 mt-1">Mark an exam as "Completed" from the Exams page to see results here</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-[300px_1fr] gap-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-500">Select Exam</h3>
              {/* Mobile toggle */}
              <button
                className="xl:hidden flex items-center gap-1 text-xs text-slate-400 hover:text-slate-200"
            ...
            <div className={`space-y-2 ${selectorOpen ? '' : 'hidden xl:block'}`}>
              ...
              {filteredExams.length === 0 && (
                <p className="text-sm text-slate-600 text-center py-8">No exams for this filter</p>
              )}
            </div>
          </div>
            ...
            {selectedExam ? (
              <div className="space-y-5">
                ...
                <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-800">
                  <div>
                    <h2 className="text-xl font-bold text-slate-100">{selectedExam.name}</h2>
                  ...
                  {selectedExam.result_published ? (
                    ...
                    <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-800 border border-slate-700 text-slate-500">
