  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-zinc-600 mt-1 text-sm flex items-center gap-2">
          {isRegular ? (
        ...
        <div className="flex items-center gap-1 bg-zinc-50 p-1 rounded-xl w-fit border border-zinc-100">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeView === 'weekly' ? 'bg-zinc-100 text-white' : 'text-zinc-600 hover:text-zinc-800'
            }`}
          >
            <Calendar size={14} />
            Weekly Classes
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeView === 'exams' ? 'bg-zinc-100 text-white' : 'text-zinc-600 hover:text-zinc-800'
            }`}
          >
            <BookOpen size={14} />
            Exam Schedule
...
function StudentRoutineSkeleton({ isRegular }: { isRegular: boolean }) {
      ...
      <div className="grid grid-cols-5 gap-3 animate-pulse">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="space-y-2">
            <div className="h-9 rounded-lg bg-zinc-50" />
            {Array.from({ length: 4 }).map((_, j) => (
              <div key={j} className="h-20 rounded-lg bg-zinc-50" />
            ))}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-2 animate-pulse">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="h-20 rounded-xl bg-zinc-50" />
      ))}
    </div>
  )
}
