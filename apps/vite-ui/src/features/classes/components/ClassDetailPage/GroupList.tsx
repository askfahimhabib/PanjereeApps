...
export function GroupList({ groups, classId }: GroupListProps) {
  ...
  if (groups.length === 0) {
    return (
      <div className="text-center p-10 text-zinc-600 border border-dashed border-zinc-100 rounded-xl">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {groups.map((group) => {
        ...
        return (
          <div
            key={group.id}
            className={`overflow-hidden rounded-xl border-2 bg-white transition-all duration-300 ${theme.border}`}
          >
