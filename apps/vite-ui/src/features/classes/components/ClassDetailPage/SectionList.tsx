...
export function SectionList({ sections, onDelete }: SectionListProps) {
  if (sections.length === 0) {
    return (
      <div className="text-center p-10 text-zinc-600 border border-dashed border-zinc-100 rounded-xl bg-zinc-50">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {sections.map((section) => {
        ...
        return (
          <div
            key={section.id}
            className="group overflow-hidden rounded-xl border border-zinc-100 bg-zinc-50 hover:bg-zinc-50 hover:border-zinc-100 transition-all duration-300"
          >
            <div className="p-5">
              {/* ── Header ───────────────────────────────────────────── */}
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-semibold text-zinc-900">Section {section.name}</h3>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {section.status === 'INACTIVE' && (
                    <span className="text-xs bg-slate-600/40 text-zinc-600 border border-zinc-100 px-2 py-0.5 rounded-full">
                      Inactive
                    </span>
                  )}
                  {onDelete && (
                    <button
                      ...
                      className="p-1.5 text-zinc-800 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
              ...
              <div className="space-y-2 mb-3">
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center text-zinc-800">
                    <Users className="w-3.5 h-3.5 mr-1.5 text-zinc-600" />
                    <span className="text-xs">Students</span>
                  </div>
                  <span className={`text-xs font-medium ${fillPct > 0.9 ? 'text-red-400' : 'text-zinc-800'}`}>
                    {section.totalStudents} / {section.capacity}
                  </span>
                </div>
              ...
              <Link
                ...
                className="flex items-center justify-between w-full px-4 py-2.5 rounded-lg bg-zinc-100 hover:bg-blue-600 text-zinc-800 hover:text-white text-sm font-medium transition-all duration-200"
              >
                View Roll List
