    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3.5">
        <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-xs">
          ...
          <p className="text-xl font-extrabold text-zinc-900 mt-1">৳{totalReceivable.toLocaleString()}</p>
          <p className="text-[11px] text-zinc-500 mt-0.5">{students.length} Total Students</p>
        </div>

        <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-xs">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Total Collected</p>
          <p className="text-xl font-extrabold text-emerald-700 mt-1">৳{totalCollected.toLocaleString()}</p>
          <p className="text-[11px] text-emerald-600 font-semibold mt-0.5">{paidStudents.length} Students Paid</p>
        </div>

        <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-xs">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Total Due / Unpaid</p>
          <p className="text-xl font-extrabold text-rose-700 mt-1">৳{totalDue.toLocaleString()}</p>
          <p className="text-[11px] text-rose-600 font-semibold mt-0.5">{dueStudents.length} Students Due</p>
        </div>

        <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-xs">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Collection Rate</p>
          <p className="text-xl font-extrabold text-indigo-700 mt-1">{collectionRate}%</p>
        ...
        <div className="px-5 py-3.5 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-zinc-800 uppercase tracking-wider">
              Unpaid / Due Students ({dueStudents.length} Students)
            </h4>
          </div>
          <span className="text-xs text-rose-700 font-bold">Total Due: ৳{totalDue.toLocaleString()}</span>
        </div>
