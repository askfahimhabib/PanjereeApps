...
export function PromoteStudentsModal({ isOpen, onClose, students, onPromote }: Props) {
  ...
  const inputCls = "w-full bg-white border border-zinc-100 rounded-xl px-4 py-2.5 text-sm text-zinc-800 focus:outline-none focus:border-blue-500 transition-colors"
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div className="relative w-full max-w-4xl bg-zinc-50 rounded-2xl border border-zinc-100 shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-zinc-900">Promote Students</h2>
            <p className="text-sm text-zinc-600">Migrate students to the next class and session</p>
          </div>
          <button onClick={onClose} className="p-2 text-zinc-600 hover:text-zinc-800 hover:bg-zinc-50 rounded-xl transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-1/3 bg-zinc-50 border-r border-zinc-100 p-6 flex flex-col gap-6 shrink-0">
            {/* Source */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-zinc-800">Select Source</h3>
              </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-zinc-800">Select Target</h3>
              </div>
              <div className="space-y-4">
                <div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="relative w-64">
                ...
                <input
                  ...
                  className="w-full bg-zinc-50 border border-zinc-100 hover:border-zinc-100 rounded-lg pl-9 pr-3 py-1.5 text-sm text-zinc-800 focus:border-blue-500 transition-colors outline-none"
                />
              </div>
            </div>

            {!sourceClass || !sourceSession ? (
              <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 gap-3 border border-dashed border-zinc-100 rounded-xl bg-white">
              ...
              <div className="flex-1 border border-zinc-100 rounded-xl overflow-hidden flex flex-col">
                <div className="grid grid-cols-[auto_1fr_1fr_auto] gap-4 p-3 bg-white border-b border-zinc-100 items-center">
                  ...
                  <span className="text-xs font-semibold text-zinc-600 uppercase">Student Name</span>
                  <span className="text-xs font-semibold text-zinc-600 uppercase">ID / Roll</span>
                  <span className="text-xs font-semibold text-zinc-600 uppercase w-20 text-center">Status</span>
                </div>
                <div className="flex-1 overflow-y-auto">
                  {eligibleStudents.map(student => (
                    <div 
                      key={student.id} 
                      className="grid grid-cols-[auto_1fr_1fr_auto] gap-4 p-3 border-b border-zinc-100 hover:bg-zinc-50 transition-colors items-center cursor-pointer"
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-zinc-800">{student.fullNameEn}</p>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-xs text-zinc-600">{student.studentId}</span>
                        <span className="text-xs text-zinc-600">Roll: {student.rollNumber}</span>
                      </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-4 border-t border-zinc-100 bg-white shrink-0 rounded-b-2xl">
          <div className="text-sm text-zinc-600">
            {selectedIds.size} student(s) selected
          </div>
          <div className="flex gap-3">
            <button onClick={onClose} className="px-5 py-2 text-sm font-medium text-zinc-800 hover:text-white transition-colors">
              Cancel
            </button>
            <button 
