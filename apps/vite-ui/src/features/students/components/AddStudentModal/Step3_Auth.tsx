...
export function Step3_Auth({ data, onChange }: Props) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-2 bg-zinc-100 hover:bg-slate-600 border border-zinc-100 rounded-lg text-zinc-800 transition-colors shrink-0"
              title="Re-generate username"
            >
              <RefreshCw size={14} />
            </button>
          </div>
          <p className="text-xs text-zinc-600 mt-1">Auto-generated from student's name. Can be changed.</p>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-2 bg-zinc-100 hover:bg-slate-600 border border-zinc-100 rounded-lg text-zinc-800 transition-colors shrink-0 text-xs whitespace-nowrap"
            >
              Auto
            </button>
          </div>
          ...
          {data.password && (
            <div className="mt-2">
              ...
              <p className="text-xs text-zinc-600 mt-1">
                Strength: <span className="text-zinc-800">{strength.label}</span>
              </p>
            </div>
          )}
        </div>
      ...
      {/* ── Login Status ──────────────────────────────── */}
      <div className={sectionCls}>
        <h3 className="text-sm font-semibold text-zinc-800 mb-4">Login Access</h3>
        <div className="flex gap-3">
          {[
            { value: 'ACTIVE', label: '✅ Active (can login)', color: 'border-emerald-500/60 bg-emerald-500/10 text-emerald-300' },
            { value: 'INACTIVE', label: '⛔ Inactive (blocked)', color: 'border-zinc-100 bg-zinc-100 text-zinc-600' },
          ].map(opt => (
            <button
              key={opt.value}
              type="button"
              ...
              className={`flex-1 py-2.5 px-4 rounded-lg border text-sm font-medium transition-all ${
                data.loginStatus === opt.value ? opt.color : 'border-zinc-100 bg-white text-zinc-600 hover:border-zinc-100'
              }`}
            >
              {opt.label}
            </button>
