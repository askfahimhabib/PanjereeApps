      ...
      {WEEKDAYS.map((day) => {
        ...
        return (
          <div key={day} className="flex flex-col gap-2">
            {/* Day Header */}
            <div className="text-center py-2 rounded-lg bg-white border border-zinc-100">
              <p className="text-xs font-semibold text-zinc-800 tracking-wide">{DAY_LABELS[day]}</p>
            </div>
            ...
            <div className="flex flex-col gap-2 min-h-[120px]">
              ...
              {!readonly && (
                <button
                  ...
                  className="w-full py-3 rounded-lg border border-dashed border-zinc-100 text-zinc-800 hover:border-blue-500/50 hover:text-blue-400 transition-all text-xl leading-none"
                >
                  +
                </button>
              )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-1 mb-1.5">
        <p className={`text-xs font-semibold leading-snug flex-1 min-w-0 ${isInactive ? 'line-through text-zinc-600' : 'text-zinc-800'} ${slot.entry_type === 'OFF_DAY' ? 'text-center py-3' : ''}`}>
        ...
        {!readonly && onDelete && (
          <button
            ...
            className="opacity-0 group-hover:opacity-100 text-zinc-800 hover:text-red-400 transition-all flex-shrink-0 -mt-0.5"
          >
            <Trash2 size={12} />
          </button>
        )}
      ...
      {slot.entry_type !== 'OFF_DAY' && (
        <>
          ...
          {/* Time */}
          <div className={`flex items-center gap-1 text-[10px] ${isInactive ? 'text-zinc-800' : 'text-zinc-600'}`}>
            <Clock size={10} />
