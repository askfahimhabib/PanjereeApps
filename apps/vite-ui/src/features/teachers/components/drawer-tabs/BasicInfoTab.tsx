...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      <h4 className="text-xs font-semibold text-zinc-600 uppercase tracking-wider border-b border-zinc-100 pb-2">{title}</h4>
...
export function BasicInfoTab({ teacher }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Address */}
      <div className="space-y-3">
        <h4 className="text-xs font-semibold text-zinc-600 uppercase tracking-wider border-b border-zinc-100 pb-2">Address</h4>
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            <MapPin size={15} className="text-zinc-600 mt-0.5 shrink-0" />
            <div>
              <p className="text-xs text-zinc-600">Present Address</p>
              <p className="text-sm text-zinc-800 font-medium">{teacher.presentAddress || '—'}</p>
            </div>
          </div>
          {teacher.permanentAddress && teacher.permanentAddress !== teacher.presentAddress && (
            <div className="flex items-start gap-3">
              <MapPin size={15} className="text-zinc-600 mt-0.5 shrink-0" />
              <div>
                <p className="text-xs text-zinc-600">Permanent Address</p>
                <p className="text-sm text-zinc-800 font-medium">{teacher.permanentAddress}</p>
              </div>
            </div>
          )}
        </div>
