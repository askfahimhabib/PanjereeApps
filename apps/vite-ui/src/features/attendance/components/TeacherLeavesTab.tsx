...
export function TeacherLeavesTab() {
  ...
  const teacherLeaves = useMemo(() => {
    return leaveStore
      .getWhere(l => l.applicantType === 'TEACHER')
      .sort((a, b) => new Date(b.appliedAt).getTime() - new Date(a.appliedAt).getTime())
  }, [refreshKey])

  const filteredLeaves = useMemo(() => {
    return teacherLeaves.filter(r => {
  ...
  }, [teacherLeaves, tab, search])

  const stats = useMemo(() => {
    return {
      all: teacherLeaves.length,
      pending: teacherLeaves.filter(r => r.status === 'PENDING').length,
      approved: teacherLeaves.filter(r => r.status === 'APPROVED').length,
      rejected: teacherLeaves.filter(r => r.status === 'REJECTED').length,
    }
  }, [teacherLeaves])
  ...
  return (
    <div className="space-y-6">
      {/* ── 1. Annual Leave Quota Policy Cards ───────────────────────────── */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="p-4 bg-white border border-zinc-200 rounded-3xl shadow-2xs flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Casual Leave (CL)</p>
            <p className="text-sm font-black text-zinc-900">14 Days / Year Quota</p>
          </div>
        </div>
        ...
        <div className="p-4 bg-white border border-zinc-200 rounded-3xl shadow-2xs flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Medical / Sick Leave (SL)</p>
            <p className="text-sm font-black text-zinc-900">14 Days / Year Quota</p>
          </div>
        </div>
        ...
        <div className="p-4 bg-white border border-zinc-200 rounded-3xl shadow-2xs flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Earned Leave (EL)</p>
            <p className="text-sm font-black text-zinc-900">10 Days / Year Quota</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-1.5 p-1 bg-zinc-100 rounded-2xl border border-zinc-200">
          {(['ALL', 'PENDING', 'APPROVED', 'REJECTED'] as const).map(s => {
            ...
            return (
              ...
              >
                <span>{s === 'ALL' ? 'All Faculty Leaves' : STATUS_CFG[s].label}</span>
                <span
      ...
      <div className="bg-white rounded-3xl border border-zinc-200 overflow-hidden shadow-xs">
        <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
          ...
          <p className="text-xs text-zinc-500 font-medium">
            Approved leaves synchronize with daily faculty attendance and payroll audit
          </p>
        </div>
        ...
        <div className="divide-y divide-zinc-100">
          {filteredLeaves.length === 0 ? (
            <div className="py-16 text-center text-zinc-500">
              <Briefcase size={36} className="mx-auto mb-2 text-zinc-300" />
              <p className="text-sm font-bold text-zinc-700">No faculty leave requests found</p>
              <p className="text-xs text-zinc-400 mt-0.5">
                Submitted faculty leaves will appear here for principal review.
              </p>
            </div>
          ) : (
      ...
      <div className="bg-white rounded-3xl border border-zinc-200 shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 bg-zinc-50/50">
          <h3 className="font-bold text-zinc-900 text-sm">
            {isReject ? 'Reject Faculty Leave' : 'Approve Faculty Leave'}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          <div className="p-3.5 bg-zinc-50 border border-zinc-200 rounded-2xl text-xs space-y-1">
            ...
            <p className="text-zinc-500">{record.designation}</p>
            <p className="text-zinc-700 font-medium">
              {record.fromDate} to {record.toDate} ({record.reason})
            </p>
          </div>
          ...
          {isReject && (
            <div>
              ...
              <textarea
                ...
                placeholder="e.g. Critical examination duties assigned..."
