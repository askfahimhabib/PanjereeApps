...
export function ExamHeldCard({ exam, onStatusChange, onDelete, onSchedule, onEnterResults, onViewSummary }: Props) {
  ...
  return (
    <div className="bg-white border border-zinc-100 rounded-2xl overflow-hidden hover:border-purple-500/30 transition-all group">
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <button
            ...
            className="text-zinc-800 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 flex-shrink-0"
          >
            <Trash2 size={16} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="bg-white rounded-lg p-3 text-center">
            <p className="text-lg font-bold text-zinc-800">{exam.total_marks}</p>
            <p className="text-[10px] text-zinc-600">Total Marks</p>
          </div>
          <div className="bg-white rounded-lg p-3 text-center">
            <p className="text-lg font-bold text-zinc-800">{schedules.length}</p>
            <p className="text-[10px] text-zinc-600">Subjects Set</p>
          </div>
        </div>
          ...
          <div className="space-y-1.5 mb-4">
            {schedules.slice(0, 3).map((s) => (
              <div key={s.id} className="flex items-center justify-between text-xs">
                <span className="text-zinc-600 truncate">{s.subjects?.name_bn ?? s.subjects?.name}</span>
                <span className="text-zinc-600 flex-shrink-0 ml-2 flex items-center gap-1">
                  <Calendar size={10} />
            ...
            {schedules.length > 3 && (
              <p className="text-[10px] text-zinc-800 pl-0.5">+ {schedules.length - 3} more...</p>
            )}
          </div>
        )}
        ...
        <div className="flex gap-2 flex-col">
          ...
          <div className="relative group/status">
            ...
            <div className="absolute bottom-full right-0 mb-1 bg-white border border-zinc-100 rounded-xl shadow-xl overflow-hidden hidden group-hover/status:block z-20 min-w-[120px]">
              {STATUS_FLOW.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-zinc-50 transition-colors ${exam.status === s ? `${c.color} font-semibold` : 'text-zinc-600'}`}
                  >
                    {c.label}
                  </button>
                )
